const CATEGORY_HOLDER = {
	
	"Pasta" : 
	{
		"Sfoglia Lorda" : ["ricotta", "parmigiano",  "https://www.cibo360.it/cucina/mondo/spoja_lorda.htm"],
		"Ravioli Ricotta e Spinaci" : ["ricotta", "spinaci", ""],
		"Ravioli alla Zucca" : ["zucca", "burro", "salvia", "https://www.lospicchiodaglio.it/ricetta/cappellacci-di-zucca"],
		"Ravioli Porro, Uova e Formaggio" : ["porri", "parmigiano", ""],
		"Cannelloni Ricotta e Spinaci" : ["ricotta", "spinaci", "parmigiano", ""],
		"Crema di Zucca e Verdure" : ["zucca", ""],
		"Lasagna ai Funghi (e Zucca)" : ["zucca", "funghi", "burro", "my-recipes/lasagna-zucca-funghi.html"],
		"Salsa di Pomodoro" : ["pomodori", ""],
		"Aglio e Olio" : [""],
		"Ricette Marchigiane" : ["", "https://www.valcesano.com/ricette-marchigiane-tradizionali"],
		
	},

	"Panetteria" : 
	{
		"Buns" : ["latte", ""],
		"Piadina" : ["https://blog.giallozafferano.it/studentiaifornelli/piadina-senza-strutto/"],
		"Scones" : [""],
		"Crespelle" : ["burro", ""],
		"Pizza" : ["mozzarella", "pomodori", "https://magazine.tognana.com/pizza-fatta-in-casa-con-il-fornetto-rotondo-sphera/"],
		"Calzone" : ["ricotta" , "mozzarella", "basilico", "prezzemolo", ""],
		"Sfoglia" : ["ricotta", "spinaci", "my-recipes/sfoglia.html"],
		"Bruschetta" : ["peperoni", "pomodori", "mozzarella", "zucchine", ""],
		"Polenta" : ["polenta", ""],
		"Focaccia" : [""],
	},

	"Frittura" : {
		"Panzerotti" : ["pomodori", "latte", ""],
		"Gnocco" : ["my-recipes/gnocco.html"],
		"Pettole" : ["", "https://blog.giallozafferano.it/fablesucre/pettole/"],
		"Mozzarella in Carrozza" : ["mozzarella", "pane", "pane grattuggiato", ""],
		"Olive Ascolane" : ["carne macinata", "olive", "pane grattuggiato", ""],
	},
		
	"Secondi" : {
		"Milchreis" : ["latte", ""],
		"Rotolo" : [""],
		"Parmigiana" : ["zucchine", "mozzarella", "parmigiano", "my-recipes/parmigiana.html"],
		"Besciamella con Verdure" : ["rosenkohl", "funghi", "zucchine", ""],
		"Sformato di Broccoli" : ["broccoli", "patate", "carote", ""],
		"Uova in Salsa di Pomodoro" : [""],
		"Ragù di Lenticchie" : ["lenticchie", "sedano", "cipolle", "carote", ""],
		"Uova alla Diavola" : [""],
		"Verdure al Forno con Uova" : ["porri", "carote", ""],
		
	},

	"Zuppe" : {
		"Zuppa di Noci" : ["noci", "panna", "https://www.cucchiaio.it/ricetta/ricetta-minestra-noci/"],
		"Zuppa d'Aglio" : ["https://www.buonissimo.it/lericette/3512_Zuppa_d_aglio"],
		"Zuppa Imperiale" : ["https://ricette.giallozafferano.it/Zuppa-imperiale.html"],
		"Stracciatella" : ["brodo", "pane grattuggiato", ""],
	},

	"Salse" : {
		"Salsa allo Yogurt" : ["yogurt", "yogurt greco", ""],
		"Pane Pnoci" : ["noci", "olive", "basilico", ""],
		"Maionese" : ["basilico", ""],
		"Paprikash" : ["peperoni", "yogurt greco", ""],
		"Tzatziki" : ["cetriolo", "yogurt greco", ""],
		"Tirokafteri" : ["peperoni", "feta", "yogurt greco", "https://erasmusu.com/it/blog-erasmus/ricette-erasmus/come-preparare-la-tradizionale-tirokafteri-salsa-greca-455871"],
		"Hummus di lenticchie" : ["lenticchie", "pomodori", ""],
		"Pesto" : ["basilico", "noci", "prezzemolo", "zucchine", ""],
		"Pesto Pistacchi, Pomodorini, Porro" : ["pistacchi", "pomodorini", "porro", ""],
		"Burro alle Erbe" : ["burro", "basilico", "prezzemolo", "https://www.ilclubdellericette.it/ricetta/burro-alle-erbe"],
		"Curry" : ["prezzemolo", "aglio", "zenzero", "curcuma", "chili" , "my-recipes/curry.html"],
		"Provenzale" : ["tonno", "mozzarella", "pomodori", ""],
		"Scappatella" : ["funghi", "pomodori", "insalata", "maionese", ""],
		"Ortolana" : ["zucchine", "uova", "grana", "pomodori",  ""],
		"Currywurst Sauce" :  ["pomodori", "brodo", "miele", "aceto di mele", "https://www.daringgourmet.com/best-german-curry-ketchup-for-currywurst/"],
	},

	"Carne" : {
		"Alla Filippo" : ["vino rosso", "panna", "cipolle", "salsiccia", ""],
		"Ravioli del Plin" : ["carote", "cipolle", "sedano", "uova", "parmigiano", "https://ricette.giallozafferano.it/Ravioli-del-plin.html"],
		"Lasagna col Ragù" : ["ragù", ""],
		"Lasagna Bianca" : [""],
	},

	"Dolci" : {
		//"Zuppa Inglese" : ["alchermes", ""],
		"Crema Pasticcera" : ["alchermes", ""],
		"Tiramisù" : ["mascarpone", "cacao amaro", "caffè", "savoiardi", ""],
		"Pere e Vino" : ["pere", "vino rosso", "https://ricette.giallozafferano.it/pere-al-vino-rosso.html"],
		"Cicerchiata" : ["miele di acacia", "vino liquoroso", "https://blog.giallozafferano.it/quasimagia/cicerchiata-ricetta-di-carnevale-marche/"],
		"Profiteroles" : ["panna fresca", "cioccolato fondente", "cioccolato bianco", "https://ricette.giallozafferano.it/Profiteroles-al-cioccolato-bianco.html"],
		"Cioccolata Calda" : ["cioccolato", "nocciole", "panna", "https://www.davidezambelli.com/ricette/cioccolata-calda/"],
		"Pancakes" : ["http://bressanini-lescienze.blogautore.espresso.repubblica.it/2015/07/06/le-ricette-scientifiche-i-pancake/"],
		"Ciambellone": ["https://blog.giallozafferano.it/rossellainpadella/ciambella-al-cioccolato-al-latte/"],
		"Nocciolata" : ["zucchero di canna", "cioccolato", "nocciole" ,"my-recipes/nutella.html"],
	},
	
	"Fondamentali" : {
		"Farina" : ["farina", ""],
		"Uova" : ["uova", ""],
		"Vegetables" : ["aglio", "cipolle", "patate", "sedano", "carote", "peperoni", "chili", "pomodori", "zucchine", "sauerkraut", ""],
		"Passata di Pomodoro" : ["passata di pomodoro", ""],
		"Banane" : ["banane", ""],
		"Latte" : ["latte", ""],
		"Olio di Oliva" : ["olio di oliva", ""],
		"Olio di Girasole" : ["olio di girasole", ""],
		"Yogurt" : ["yogurt", ""],
		"Burro" : ["burro", ""],
		"Lievito di birra" : ["lievito di birra", ""],
		"Lievito per dolci" : ["lievito per dolci", ""],
		"Cacao in polvere" : ["cacao in polvere", ""],
		"Zucchero" : ["zucchero", ""],
		"Sale" : ["sale", ""],
		"Pepe" : ["pepe", ""],
		"Rosmarino" : ["rosmarino", ""],
		"Timo" : ["timo", ""],
		"Noce Moscata" : ["noce moscata", ""],
		"Scottex" : ["scottex", ""],
		"Fazzoletti" : ["fazzoletti", ""],
		"Bagnoschiuma/Sapone" : ["bagnoschiuma/sapone", ""],
		"Dentifricio" : ["dentifricio", ""],
		"Detersivo" : ["detersivo", ""],
		"Spugne" : ["spugne", ""],
		"Guanti" : ["guanti", ""],
		"Carta Igienica" : ["carta igienica", ""],
		"Spray Pulizia" : ["spray pulizia", ""],
		"Spazzatura" : ["spazzatura", ""],
		"Lettiera" : ["lettiera", ""],
		"Cibo per Gattino" : ["cibo per gattino", ""],
		"Roba per Lavastoviglie" : ["roba per lavastoviglie", ""],
	},

	"Siti di Riferimento" : {
		"Gamoto.net, sito del dizionario di Latino" : ["https://www.gamoto.net/ricette-cucina/"]
	}
}

